(function () {
    ...
    const TokenGenerator = {
        ...
        drawToken: function (name) {
            ...
            const lines = name.trim().split(' ').map(str => str.length > 12 ? str.substring(0, 12) + '.' : str);
        ...
        generateTokenFile(creatureName) {
            ...
            return this.dataURLtoFile(tokenDataUrl, creatureName + ".png");
        },
        downloadFile(filename, name) {
            const element = document.createElement('a');
            element.setAttribute('href', this.generateTokenDataUrl(name));
    ...
    const Roll20Page = {
        ...
        noCharacterSheetsHandoutsOpened: function() {
            return $('div.characterdialog iframe').length == 0;
        },
    ...
    async function importCreature(roll20Sheet) {
        try {
            if (Roll20Page.noCharacterSheetsHandoutsOpened()) {
